...
class Cart:
	...
	def get(user_id):
		rows = app.db.execute('''
SELECT c.user_id, p.id, c.product_id, p.name, c.quantity, p.price, c.ts
...
''',
	...
	def submit_aggregate(total_amount):
		res = app.db.execute('''
INSERT INTO Order_info_aggregate(total_amount)
VALUES (:total_amount)
RETURNING id
''',
	...
	@staticmethod
	def submit_detail(order_id, buyer_id, cart):
		for item in cart:
			app.db.execute('''
INSERT INTO Order_info_detail(order_id, buyer_id, seller_id, product_id, quantity, price)
